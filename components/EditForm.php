...
final class EditForm extends ReactForm implements IEditFormFactory {
    ...
    private function getValidators(array $column, $name) {
        $validators = [];
        false == $column['nullable'] || preg_match('/\@required/', $column['vendor']['Comment']) ? $validators['required'] = ucfirst($this->translatorModel->translate($column['name']) . ' ' . $this->translatorModel->translate('is required.')) : null;
