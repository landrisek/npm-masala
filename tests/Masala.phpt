...
final class MasalaTest extends TestCase {
    ...
    public function testAttached(): void {
        ...
        Assert::false(empty($settings = (array) json_decode($this->mockFacade->getUser()->getIdentity()->getData()[$config['masala']['settings']])), 'Test user does not have settings.');
        ...
        foreach ($presenters as $class => $presenter) {
            if(isset($this->container->parameters['masala']['tests']['presenters'][$class])) {
                $testParameters = $this->container->parameters['masala']['tests']['presenters'][$class];
            } else if(isset($this->container->parameters['masala']['tests']['parameters'])) {
                $testParameters = $this->container->parameters['masala']['tests']['parameters'];
            } else {
                $testParameters = [];
            }
            ...
            Assert::true(is_object($grid = $this->mockFacade->getPrivateProperty($this->class, 1)), 'Masala builder is not set.');
            ...
            Assert::true(is_object($gridFactory = $this->mockFacade->getPrivateProperty($this->class, 2)), 'IGridFactory is not set.');
            ...
            Assert::true(is_object($filterForm = $this->mockFacade->getPrivateProperty($gridFactory, 3)), 'FilterForm is not set.');
    ...
    public function testHandleExport(): void {
        Assert::same(null, $this->mockFacade->setPost(['Offset'=>1]), 'MockFacade:setPost succeed but it does return something. Do you wish to modify test?');
    ...
    public function testHandleRun(): void {
        ...
        /*Assert::false(empty($excluded = $this->container->parameters['masala']['excluded']), 'No tables to excluded. Do you wish to modify test?');
